...
.PHONY: all build-frontend build-frontend-default build-frontend-classic build-all-frontends build-upstream-frontends start-backend dev dev-api dev-api-rebuild dev-web dev-web-default dev-web-classic reset-setup
...
build-frontend-default:
	...
	@cd ./web && bun install --frozen-lockfile
	@cd $(FRONTEND_DEFAULT_DIR) && DISABLE_ESLINT_PLUGIN='true' VITE_REACT_APP_VERSION=$(cat ../../VERSION) bun run build
...
build-upstream-frontends: build-frontend-default build-frontend-classic
...
dev-web:
	@echo "Starting AtomPump frontend dev server..."
	@echo "AtomPump frontend: http://localhost:$(DEV_FRONTEND_ATOMPUMP_PORT)"
	@cd ./web && bun install
	@cd $(FRONTEND_DIR) && bun run dev -- --host 0.0.0.0 --port $(DEV_FRONTEND_ATOMPUMP_PORT)

dev-web-default:
	@echo "Starting default frontend dev server..."
	@cd ./web && bun install
	@cd $(FRONTEND_DEFAULT_DIR) && bun run dev -- --host 0.0.0.0 --port $(DEV_FRONTEND_DEFAULT_PORT)
